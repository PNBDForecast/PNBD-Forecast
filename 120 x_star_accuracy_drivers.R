...
# MAPE DRIVERS
#############################################################
# RMSPE besitzt nur ein adj. R² von 0.6131 => nicht geeignet

  load("results/101_x_star_deviations.Rdata")
...
#---- 13 weeks-----------
  ...
  lm.sim.13 <- lm(MAPE[[1]]$sim_med ~ ., data= predictors.het)
  lm.ind.13 <- lm(MAPE[[1]]$ind_med ~ ., data = predictors.het)
  lm.MLE.13 <- lm(MAPE[[1]]$MLE ~ ., data = predictors.het)
  lm.heur.13 <- lm(MAPE[[1]]$heur ~., data=predictors.het)
...
#---- 26 weeks-----------
  lm.sim.26 <- lm(MAPE[[2]]$sim_med ~ ., data= predictors.het)
  lm.ind.26 <- lm(MAPE[[2]]$ind_med ~ ., data = predictors.het)
  lm.MLE.26 <- lm(MAPE[[2]]$MLE ~ ., data = predictors.het)
  lm.heur.26 <- lm(MAPE[[2]]$heur ~., data=predictors.het)
...
#----52 weeks -------------
  lm.sim.52 <- lm(MAPE[[3]]$sim_med ~ ., data= predictors.het)
  lm.ind.52 <- lm(MAPE[[3]]$ind_med ~ ., data = predictors.het)
  lm.MLE.52 <- lm(MAPE[[3]]$MLE ~ ., data = predictors.het)
  lm.heur.52 <- lm(MAPE[[3]]$heur ~., data=predictors.het)
...
save(lm.13, lm.26, lm.52, file="results/120_x_star_regression_models.Rdata")  
